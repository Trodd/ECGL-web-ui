...
def migrate_matches():
    """Migrate matches from the Google Sheet into the DB (no season column)."""
    try:
    ...
    for row in rows:
        ...
        # --- Parse discord/unix timestamps safely ---
        def safe_parse_date(s):
            if not s or s.upper() == "TBD":
                return None
            s = s.strip()
            try:
                # Discord timestamp: <t:1752026400:f>
        ...
        # --- Resolve team IDs safely ---
        def lookup_team(name):
            if not name:
                return None
            cur.execute("SELECT id FROM teams WHERE LOWER(name)=LOWER(%s)", (name.strip(),))
            r = cur.fetchone()
            return r[0] if r else None

        team_a_id = lookup_team(team_a)
        team_b_id = lookup_team(team_b)
        winner_id = lookup_team(winner_name)
        loser_id = lookup_team(loser_name)

        # --- Allow partial or orphaned matches ---
        if not team_a_id and not team_b_id:
            print(f"⚠️ Both teams missing for {match_code} ({team_a} vs {team_b}) — migrating anyway as historical record.")
        elif not team_a_id or not team_b_id:
            print(f"⚠️ Partial match for {match_code}: missing "
                f"{team_a if not team_a_id else team_b}")
        ...
        """, (
            match_code, team_a_id, team_b_id,
            scheduled_date, status, winner_id, loser_id
        ))
        ...
        if not team_a_id or not team_b_id:
            print(f"⚠️ Partial match saved for {match_code}: missing "
                  f"{team_a if not team_a_id else team_b}")
            continue
...
def migrate_player_history():
    ...
    # clear current season before snapshot
