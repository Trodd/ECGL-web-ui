...
export default function Register() {
  ...
  async function handleRequestJoin(teamName) {
    ...
    // --- UI ---
    if (!me) {
      return <p>🔑 Please log in with Discord to register.</p>;
    }
  ...
  return (
    <div>
      ...
      {/* Show register form if not registered OR marked inactive */}
      {!me?.registered ? (
        <form onSubmit={handleRegister} className="d-flex flex-column gap-3">
          {/* ⚠️ Platform Requirement Notice */}
          <div 
            className="alert alert-warning small mb-2"
            style={{ maxWidth: "500px" }}  // adjust width
          >
            ...
            ✅ You must play via <b>SteamVR</b> or <b>Oculus PC</b> (Air Link / Link cable).
          </div>
        ...
        <div>
          <p>
            ✅ You are registered as <b>{me.role}</b>
          </p>
          ...
          <button className="btn btn-danger mt-3 w-auto d-inline-block" onClick={handleUnregister}>
            Unregister
          </button>
        </div>
      )}
